...
// MoveJ ACTION SERVER for KUKA LBR-IIWA Robot.
...
#include "ros2_data/action/move_jiiwa.hpp"
#include "ros2_data/msg/joint_pose_iiwa.hpp"
...
{
public:
    using MoveJiiwa = ros2_data::action::MoveJiiwa;
    using GoalHandle = rclcpp_action::ServerGoalHandle<MoveJiiwa>;

    explicit ActionServer(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
    : Node("MoveJiiwa_ActionServer", options)
    {

        action_server_ = rclcpp_action::create_server<MoveJiiwa>(
            this,
            "/MoveJiiwa",
...
private:
    rclcpp_action::Server<MoveJiiwa>::SharedPtr action_server_;
    
    // Function that checks the goal received, and accepts it accordingly:
    rclcpp_action::GoalResponse handle_goal(
        const rclcpp_action::GoalUUID & uuid,
        std::shared_ptr<const MoveJiiwa::Goal> goal)
    {
        auto JointGoal = ros2_data::msg::JointPoseIiwa();
        JointGoal =  goal->goal;
    ...
    {
        RCLCPP_INFO(this->get_logger(), "Starting MoveJiiwa motion to desired waypoint...");
        
        rclcpp::Rate loop_rate(0.5);
        
        // Obtain input value (goal -- JointPoseiiwa):
        const auto goal = goal_handle->get_goal();
        auto JointGoal = goal->goal;
        
        // FEEDBACK?
        // No feedback needed for MoveJiiwa Action Calls.
        // No loop needed for MoveJiiwa Action Calls.
        
        // Declare RESULT:
        auto result = std::make_shared<MoveJiiwa::Result>();
        ...
        double j7upper = 175;
        double j7lower = -175;
        if (j7 <= j7upper && j7 >= j7lower && LimitCheck == false) {
            // Do nothing, check complete.
        } else {
            LimitCheck = true;
        }
        ...
        if (LimitCheck == false){
            ...
            // Joint model group:
            const moveit::core::JointModelGroup* joint_model_group = move_group_interface.getCurrentState()->getJointModelGroup("iiwa_arm");
            ...
            if(success) {
                ...
                RCLCPP_INFO(this->get_logger(), "KUKA LBR-IIWA Robot - MoveJiiwa: Planning successful!");
                move_group_interface.move();
                
                // Do if GOAL CANCELLED:
                if (goal_handle->is_canceling()) {
                    RCLCPP_INFO(this->get_logger(), "Goal canceled.");
                    result->result = "MoveJiiwa:CANCELED";
                    goal_handle->canceled(result);
                    return;
                } else {
                    RCLCPP_INFO(this->get_logger(), "KUKA LBR-IIWA Robot - MoveJiiwa: Movement executed!");
                    result->result = "MoveJiiwa:SUCCESS";
                    goal_handle->succeed(result);
                }

            } else {
                RCLCPP_INFO(this->get_logger(), "KUKA LBR-IIWA Robot - MoveJiiwa: Planning failed!");
                result->result = "MoveJiiwa:FAILED";
                goal_handle->succeed(result);
            }    

        } else {

            RCLCPP_INFO(this->get_logger(), "KUKA LBR-IIWA Robot - MoveJiiwa: Planning failed, JOINT LIMITS exceeded!");
            result->result = "MoveJiiwa:FAILED";
            goal_handle->succeed(result);
...
int main(int argc, char ** argv)
{
  ...
  auto const node2= std::make_shared<rclcpp::Node>(
      "iiwa_moveit2_interface", rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true));
  ...
  move_group_interface = MoveGroupInterface(node2, "iiwa_arm");
  ...
  // MOVE IIWA Robot to HOME POSITION - to avoid singularity when launching GAZEBO:
    const moveit::core::JointModelGroup* joint_model_group = move_group_interface.getCurrentState()->getJointModelGroup("iiwa_arm");
    moveit::core::RobotStatePtr current_state = move_group_interface.getCurrentState(10);
    std::vector<double> joint_group_positions;
    current_state->copyJointGroupPositions(joint_model_group, joint_group_positions);
    joint_group_positions[0] = 0.0 * k;
    joint_group_positions[1] = 0.0 * k;
    joint_group_positions[2] = -90.0 * k;
    joint_group_positions[3] = 90.0 * k;
    joint_group_positions[4] = 0.0 * k;
    joint_group_positions[5] = -90.0 * k; 
    joint_group_positions[6] = 0.0 * k;
    move_group_interface.setJointValueTarget(joint_group_positions);
    moveit::planning_interface::MoveGroupInterface::Plan my_plan;
    move_group_interface.move();
