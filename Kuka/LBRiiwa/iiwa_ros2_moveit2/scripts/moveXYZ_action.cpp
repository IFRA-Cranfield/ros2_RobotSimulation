...
// MoveXYZ ACTION SERVER for KUKA LBR-IIWA Robot.
...
private:
    ...
    void execute(const std::shared_ptr<GoalHandle> goal_handle)
    {
        ...
        if(success) {
            ...
            RCLCPP_INFO(this->get_logger(), "KUKA LBR-IIWA - MoveXYZ: Planning successful!");
            ...
            if (goal_handle->is_canceling()) {
                ...
                RCLCPP_INFO(this->get_logger(), "KUKA LBR-IIWA - MoveXYZ: Movement executed!");
                result->result = "MoveXYZ:SUCCESS";
                goal_handle->succeed(result);
            }

        } else {
            RCLCPP_INFO(this->get_logger(), "KUKA LBR-IIWA - MoveXYZ: Planning failed!");
...
int main(int argc, char ** argv)
{
  ...
  auto const node2= std::make_shared<rclcpp::Node>(
      "iiwa_moveit2_interface_MoveXYZ", rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true));
