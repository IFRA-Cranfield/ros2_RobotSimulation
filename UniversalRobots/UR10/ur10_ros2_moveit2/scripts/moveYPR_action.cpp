...
// MoveYPR ACTION SERVER for UR10 Robot.
...
private:
    ...
    void execute(const std::shared_ptr<GoalHandle> goal_handle)
    {
        ...
        if(success) {
            ...
            RCLCPP_INFO(this->get_logger(), "UR10 - MoveYPR: Planning successful!");
            ...
            if (goal_handle->is_canceling()) {
                ...
                RCLCPP_INFO(this->get_logger(), "UR10 - MoveYPR: Movement executed!");
                result->result = "MoveYPR:SUCCESS";
                goal_handle->succeed(result);
            }

        } else {
            RCLCPP_INFO(this->get_logger(), "UR10 - MoveYPR: Planning failed!");
...
int main(int argc, char ** argv)
{
  ...
  auto const node2= std::make_shared<rclcpp::Node>(
      "ur10_moveit2_interface_MoveYPR", rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true));
