...
// MoveR ACTION SERVER for UR3 Robot.
...
private:
    ...
    void execute(const std::shared_ptr<GoalHandle> goal_handle)
    {
        ...
        // Joint model group:
        const moveit::core::JointModelGroup* joint_model_group = move_group_interface.getCurrentState()->getJointModelGroup("ur3_robot");
        ...
        if (LimitCheck == false){
            ...
            if(success) {
                ...
                RCLCPP_INFO(this->get_logger(), "UR3 - MoveR: Planning successful!");
                ...
                if (goal_handle->is_canceling()) {
                    ...
                    RCLCPP_INFO(this->get_logger(), "UR3 - MoveR: Movement executed!");
                    result->result = "MoveR:SUCCESS";
                    goal_handle->succeed(result);
                }

            } else {
                RCLCPP_INFO(this->get_logger(), "UR3 - MoveR: Planning failed!");
            ...
            RCLCPP_INFO(this->get_logger(), "UR3 - MoveR: Planning failed, JOINT LIMITS exceeded!");
            result->result = "MoveR:FAILED";
            goal_handle->succeed(result);

        } else {

            RCLCPP_INFO(this->get_logger(), "UR3 - MoveR: Planning failed, JOINT NAME NOT VALID!");
            result->result = "MoveR:FAILED";
            goal_handle->succeed(result);
...
int main(int argc, char ** argv)
{
  ...
  auto const node2= std::make_shared<rclcpp::Node>(
      "ur3_moveit2_interface", rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true));
  ...
  move_group_interface = MoveGroupInterface(node2, "ur3_robot");
