...
# panda_interface.launch.py:
# Launch file for the Panda Robot GAZEBO + MoveIt!2 SIMULATION (+ Robot/Gripper triggers) in ROS2 Foxy:
...
def generate_launch_description():
    ...
    load_panda_handleft_controller = ExecuteProcess(
        cmd=['ros2', 'control', 'load_start_controller', 'panda_handleft_controller'],
        output='screen'
    )
    load_panda_handright_controller = ExecuteProcess(
        cmd=['ros2', 'control', 'load_start_controller', 'panda_handright_controller'],
        output='screen'
    )
    ...
    robot_description_semantic_config = load_file(
        "panda_ros2_moveit2", "config/panda.srdf"
    )
    ...
    kinematics_yaml = load_yaml(
        "panda_ros2_moveit2", "config/kinematics.yaml"
    )
    ...
    ompl_planning_yaml = load_yaml(
        "panda_ros2_moveit2", "config/ompl_planning.yaml"
    )
    ompl_planning_pipeline_config["move_group"].update(ompl_planning_yaml)

    # MoveIt!2 Controllers:
    moveit_simple_controllers_yaml = load_yaml(
        "panda_ros2_moveit2", "config/panda_controllers.yaml"
    )
    moveit_controllers = {
    ...
    rviz_base = os.path.join(get_package_share_directory("panda_ros2_moveit2"), "launch")
