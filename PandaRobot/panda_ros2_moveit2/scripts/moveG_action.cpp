...
// MoveG ACTION SERVER for Panda Robot.
...
private:
    ...
    void execute(const std::shared_ptr<GoalHandle> goal_handle)
    {
        ...
        // Check if INPUT GRIPPER VALUE is within the GRIPPER LIMIT VALUE:
        ...
        if (LimitCheck == false){
            ...
            // Joint model group:
            const moveit::core::JointModelGroup* joint_model_group = move_group_interface.getCurrentState()->getJointModelGroup("hand");
            ...
            if(success) {
                ...
                RCLCPP_INFO(this->get_logger(), "Panda Gripper - MoveG: Planning successful!");
                ...
                if (goal_handle->is_canceling()) {
                    ...
                    RCLCPP_INFO(this->get_logger(), "Panda Gripper - MoveG: Movement executed!");
                    result->result = "MoveG:SUCCESS";
                    goal_handle->succeed(result);
                }

            } else {
                RCLCPP_INFO(this->get_logger(), "Panda Gripper - MoveG: Planning failed!");
            ...
            RCLCPP_INFO(this->get_logger(), "Panda Robot - MoveG: Planning failed, GRIPPER LIMIT exceeded!");
            result->result = "MoveG:FAILED";
            goal_handle->succeed(result);
...
int main(int argc, char ** argv)
{
  ...
  auto const node2= std::make_shared<rclcpp::Node>(
      "pandagripper_moveit2_interface", rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true));
  ...
  move_group_interface = MoveGroupInterface(node2, "hand");
