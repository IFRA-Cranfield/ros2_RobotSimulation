...
// MoveL ACTION SERVER for Panda Robot.
    ...
    void execute(const std::shared_ptr<GoalHandle> goal_handle)
    {
        ...
        if(success1) {
            ...
            if (goal_handle->is_canceling()) {
                ...
                if(success2) {
                    RCLCPP_INFO(this->get_logger(), "PANDA - MoveL: Execution successful!");
                    result->result = "MoveL:SUCCESS";
                    goal_handle->succeed(result);
                } else {
                    RCLCPP_INFO(this->get_logger(), "PANDA - MoveL: Execution failed!");
            ...
            RCLCPP_INFO(this->get_logger(), "PANDA - MoveL: Planning failed!");
...
int main(int argc, char ** argv)
{
  ...
  auto const node2= std::make_shared<rclcpp::Node>(
      "panda_moveit2_interface_MoveL", rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true));
