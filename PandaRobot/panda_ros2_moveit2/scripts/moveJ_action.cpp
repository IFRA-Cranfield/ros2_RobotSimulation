...
// MoveJ ACTION SERVER for Panda Robot.
...
#include "ros2_data/action/move_jpanda.hpp"
#include "ros2_data/msg/joint_pose_panda.hpp"
...
{
public:
    using MoveJpanda = ros2_data::action::MoveJpanda;
    using GoalHandle = rclcpp_action::ServerGoalHandle<MoveJpanda>;

    explicit ActionServer(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
    : Node("MoveJpanda_ActionServer", options)
    {

        action_server_ = rclcpp_action::create_server<MoveJpanda>(
            this,
            "/MoveJpanda",
...
private:
    rclcpp_action::Server<MoveJpanda>::SharedPtr action_server_;
    
    // Function that checks the goal received, and accepts it accordingly:
    rclcpp_action::GoalResponse handle_goal(
        const rclcpp_action::GoalUUID & uuid,
        std::shared_ptr<const MoveJpanda::Goal> goal)
    {
        auto JointGoal = ros2_data::msg::JointPosePanda();
        JointGoal =  goal->goal;
    ...
    {
        RCLCPP_INFO(this->get_logger(), "Starting MoveJpanda motion to desired waypoint...");
        
        rclcpp::Rate loop_rate(0.5);
        
        // Obtain input value (goal -- JointPosePanda):
        const auto goal = goal_handle->get_goal();
        auto JointGoal = goal->goal;
        
        // FEEDBACK?
        // No feedback needed for MoveJpanda Action Calls.
        // No loop needed for MoveJpanda Action Calls.
        
        // Declare RESULT:
        auto result = std::make_shared<MoveJpanda::Result>();
        ...
        if (LimitCheck == false){
            ...
            if(success) {
                ...
                RCLCPP_INFO(this->get_logger(), "Panda Robot - MoveJpanda: Planning successful!");
                move_group_interface.move();
                
                // Do if GOAL CANCELLED:
                if (goal_handle->is_canceling()) {
                    RCLCPP_INFO(this->get_logger(), "Goal canceled.");
                    result->result = "MoveJpanda:CANCELED";
                    goal_handle->canceled(result);
                    return;
                } else {
                    RCLCPP_INFO(this->get_logger(), "Panda Robot - MoveJpanda: Movement executed!");
                    result->result = "MoveJpanda:SUCCESS";
                    goal_handle->succeed(result);
                }

            } else {
                RCLCPP_INFO(this->get_logger(), "Panda Robot - MoveJpanda: Planning failed!");
                result->result = "MoveJpanda:FAILED";
                goal_handle->succeed(result);
            }    

        } else {

            RCLCPP_INFO(this->get_logger(), "Panda Robot - MoveJpanda: Planning failed, JOINT LIMITS exceeded!");
            result->result = "MoveJpanda:FAILED";
            goal_handle->succeed(result);
...
int main(int argc, char ** argv)
{
  ...
  auto const node2= std::make_shared<rclcpp::Node>(
      "panda_moveit2_interface", rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true));
  ...
  move_group_interface = MoveGroupInterface(node2, "panda_arm");
